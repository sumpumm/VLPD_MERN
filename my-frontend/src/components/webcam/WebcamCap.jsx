...
const WebcamCap = () => {
    ...
    const capture = useCallback(() => {
      if (webcamRef.current) {
        ...
        .then(response => {
          const detectedPlates = response.data.license_plate;
          setDetected(response.data.annotated_image);
          setPlates(response.data.license_plate);
          axios.post("http://localhost:1000/api/v2/save",{ plates: detectedPlates })
          setIsLoading(false);
        });
      }
    }, [webcamRef]);
    ...
    if (!isLoggedIn) {
      return (
        <div className="flex flex-col justify-center items-center h-screen bg-gray-900 text-white p-6">
          <h1 className="text-3xl font-bold mb-4">You do not have access to the camera</h1>
    ...
    {isLoading ? ( <p className="text-white">Processing video... please wait.</p>) : plates.length > 0 ? (
